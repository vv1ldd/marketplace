...
use App\Services\SolanaWalletPreviewEnricher;
...
class SolanaSettlementAdapter implements SettlementAdapter
{
    use DeclinesSettlementProofVerification;

    private const ADAPTER_KEY = 'solana';

    public function __construct(
        private readonly SettlementNetworkRegistry $settlementNetworks,
        private readonly WalletBindingService $bindings,
        private readonly SolanaWalletPreviewEnricher $walletPreviewEnricher,
    ...
    public function healthCheck(): array
    {
        $network = $this->settlementNetworks->network(self::ADAPTER_KEY);
        $failures = [];
        $expectedGenesis = (string) config(
            'blockchain_networks.networks.solana.expected_genesis_hash',
        ...
        $checks = [
            ...
            'expected_genesis_hash' => $expectedGenesis,
        ];
        ...
        if ($checks['rpc_configured'] && $checks['rpc_enabled']) {
            try {
                $health = $this->solana->getHealth((string) $network->rpcUrl);
                $genesis = $this->solana->getGenesisHash((string) $network->rpcUrl);
            } catch (\Throwable) {
                $health = null;
                $genesis = null;
            }

            $checks['rpc_health'] = $health;
            $checks['rpc_genesis_hash'] = $genesis;
            $checks['rpc_reachable'] = $health === 'ok' && is_string($genesis) && $genesis !== '';
            $checks['genesis_matches'] = is_string($genesis) && hash_equals($expectedGenesis, $genesis);

            if (! $checks['rpc_reachable']) {
                $failures[] = SettlementAdapterHealthCodes::RPC_ERROR;
            } elseif (! $checks['genesis_matches']) {
        ...
        $healthy = $checks['adapter_registered']
            && ($checks['adapter_enabled'] === false
                || ($checks['crypto_rails_enabled']
                    && ($checks['rpc_enabled'] === false || ($checks['rpc_reachable'] && $checks['genesis_matches']))
